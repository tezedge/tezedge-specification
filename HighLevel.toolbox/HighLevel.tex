...
\@x{}\moduleLeftDash\@xx{ {\MODULE} HighLevel}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
This is high-level model of the handshaking and bootstrapping processes
 used in the \ensuremath{Tezos} \ensuremath{P2P} layer. A node that wishes to
 join the network will
 request peers from the \ensuremath{DNS} and then make secure connections
 with established
 nodes in order to download the current state of the \ensuremath{Tezos}
 blockchain.
...
\@x{ {\EXTENDS} Utils}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS} NumNodes ,\,\@s{49.19}}%
\@y{\@s{0}%
 number of established nodes
}%
\@xx{}%
\@x{\@s{54.75} NumJoins ,\,\@s{51.89}}%
\@y{\@s{0}%
 number of joining nodes
}%
\@xx{}%
\@x{\@s{54.75} ValidStates ,\,\@s{47.43}}%
\@y{\@s{0}%
 set of valid states
}%
\@xx{}%
\@x{\@s{54.75} peerThreshold ,\,\@s{35.74}}%
\@y{\@s{0}%
 threshold of peers to exceed before handshaking
}%
\@xx{}%
\@x{\@s{54.75} connectionThreshold\@s{13.10}}%
\@y{\@s{0}%
 threshold of connections to exceed before bootstrapping
}%
\@xx{}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{ValidStates}: set of all valid states
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- joining: set of nodes who want to join the network
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{peerThreshold}: number of peers need before handshaking
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - \ensuremath{connectionThreshold}: number of connections need before
 bootstrapping
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES} state ,\,\@s{14.98}}%
\@y{\@s{0}%
 seq of states for the given node
}%
\@xx{}%
\@x{\@s{51.42} joined ,\,\@s{9.08}}%
\@y{\@s{0}%
 set of joining nodes who have joined successfully
}%
\@xx{}%
\@x{\@s{51.42} peers ,\,\@s{13.76}}%
\@y{\@s{0}%
 set of peers for the given node
}%
\@xx{}%
\@x{\@s{51.42} phase ,\,\@s{11.40}}%
\@y{\@s{0}%
 current phase of given jooining node
}%
\@xx{}%
\@x{\@s{51.42} secured ,\,\@s{4.1}}%
\@y{\@s{0}%
 set of nodes with which the given node has a secure connection
}%
\@xx{}%
\@x{\@s{51.42} sent ,\,\@s{17.60}}%
\@y{\@s{0}%
 join : set of messages sent to a given joining node
}%
\@xx{}%
\@x{\@s{93.71}}%
\@y{\@s{0}%
...
\@xx{}%
\@x{\@s{51.42} recv\@s{24.22}}%
\@y{\@s{0}%
 join : seq of messages received by a given joining node
}%
\@xx{}%
\@x{\@s{93.71}}%
\@y{\@s{0}%
...
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ vars \.{\defeq} {\langle} state ,\, joined ,\, peers ,\, phase ,\,
 secured ,\, sent ,\, recv {\rangle}}%
\@pvspace{8.0pt}%
\@x{ nodes \.{\defeq} 1 \.{\dotdot} NumNodes}%
\@pvspace{8.0pt}%
\@x{ joining \.{\defeq} 1 \.{\dotdot} NumJoins}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{peerThreshold} defines the lower bound of peers required
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{connectionThreshold} defines the lower bound of secure
 connections required
}%
\@xx{}%
 \@x{ {\ASSUME} ( peerThreshold \.{\in} Nat ) \.{\land} ( peerThreshold \.{>}
 0 )}%
 \@x{ {\ASSUME} ( connectionThreshold \.{\in} Nat ) \.{\land} (
 connectionThreshold \.{>} 0 )}%
\@x{ {\ASSUME} connectionThreshold \.{\leq} peerThreshold}%
\@x{ {\ASSUME} peerThreshold \.{\leq} NumNodes}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Variables:
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- state: record of each node\mbox{'}s view of the blockchain state
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - joined: set of nodes joining that have successfully joined the
 \ensuremath{Tezos} network
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - peers: tuple of the set of peers each joining node obtains from the
 \ensuremath{DNS
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - secured: record of tuples of \ensuremath{nodes\.{'}} sets of secure
 connections
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 The nodes are already connected to the network and have a valid blockchain
 state.
 The joining set represents nodes that are joining the network. They must do:
\end{cpar}%
\begin{cpar}{0}{T}{F}{2.5}{0}{}%
- \ensuremath{Init}: get peers from \ensuremath{DNS
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - \ensuremath{Handshake}: once peers have been obtained, make secure
 connections with peers
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - \ensuremath{Bootstrap}: once connections have been made, request state from
 these peers
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{Join}: once a valid state has been obtained, join the network
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
...
\@x{}%
\@y{\@s{0}%
 Subsets of \ensuremath{S} which contain at least \ensuremath{n} elements
}%
\@xx{}%
 \@x{ SubsetsOfSmallestSize ( S ,\, n ) \.{\defeq} \{ s \.{\in} {\SUBSET} S
 \.{:} Cardinality ( s ) \.{\geq} n \}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Choose a subset of \ensuremath{S} which contains at least \ensuremath{n}
 elements
}%
\@xx{}%
 \@x{ PickOfSize ( S ,\, n ) \.{\defeq} Pick ( SubsetsOfSmallestSize ( S ,\, n
 ) )}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 \ensuremath{S} and \ensuremath{T} are disjoint
}%
\@xx{}%
\@x{ disjoint ( S ,\, T ) \.{\defeq} S \.{\cap} T \.{=} \{ \}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@x{}%
\@y{\@s{0}%
 Set of non-empty sequences of elements of \ensuremath{S
}}%
\@xx{}%
 \@x{ NonEmptySeq ( S ) \.{\defeq} Seq ( S ) \.{\,\backslash\,} \{ {\langle}
 {\rangle} \}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Choose a singleton sequence of \ensuremath{Seq(S)
}}%
\@xx{}%
 \@x{ PickOneSeq ( S ) \.{\defeq} Pick ( \{ {\langle} t {\rangle} \.{:} t
 \.{\in} ValidStates \} )}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
...
\@x{}%
\@y{\@s{0}%
 Joining nodes in \ensuremath{init} only request peers from \ensuremath{DNS}.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Joining nodes in handshaking only attempt to make connections with peers
 (established nodes).
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Handshaking nodes are \ensuremath{peerSaturated}.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Joining nodes in bootstrapping only request state from connected established
 nodes.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Bootstrapping nodes are \ensuremath{peerSaturated} and
 \ensuremath{connectionSaturated}.
}%
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ init \.{\defeq} \{ j \.{\in} joining \.{:} phase [ j ] \.{=}\@w{init}
 \}}%
\@pvspace{8.0pt}%
 \@x{ handshaking \.{\defeq} \{ j \.{\in} joining \.{:} phase [ j ]
 \.{=}\@w{handshake} \}}%
\@pvspace{8.0pt}%
 \@x{ bootstrapping \.{\defeq} \{ j \.{\in} joining \.{:} phase [ j ]
 \.{=}\@w{bootstrap} \}}%
\@pvspace{8.0pt}%
 \@x{ PossiblePhases \.{\defeq} \{\@w{init} ,\,\@w{handshake}
 ,\,\@w{bootstrap} ,\,\@w{joined} \}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Peers
...
\@xx{}%
 \@x{ peerSaturated ( j ) \.{\defeq} Cardinality ( peers [ j ] ) \.{\geq}
 peerThreshold}%
\@pvspace{8.0pt}%
\@x{ connected ( j ,\, n ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} n \.{\in} secured . join [ j ]}%
\@x{\@s{16.4} \.{\land} j\@s{1.87} \.{\in} secured . node [ n ]}%
\@pvspace{8.0pt}%
 \@x{ connections ( j ) \.{\defeq} \{ n \.{\in} nodes \.{:} connected ( j ,\,
 n ) \}}%
\@pvspace{8.0pt}%
 \@x{ connectionSaturated ( j ) \.{\defeq} Cardinality ( connections ( j ) )
 \.{\geq} connectionThreshold}%
\@pvspace{8.0pt}%
 \@x{ connectionSaturatedFrom ( n ,\, S ) \.{\defeq} Cardinality ( secured .
 node [ n ] ) \.{\geq} connectionThreshold}%
\@pvspace{8.0pt}%
\@x{ saturatedAndBootstrapped ( j ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} j \.{\in} bootstrapping}%
\@x{\@s{16.4} \.{\land} peerSaturated ( j )}%
\@x{\@s{16.4} \.{\land} connectionSaturated ( j )}%
\@x{\@s{16.4} \.{\land} state . join [ j ] \.{\neq} {\langle} {\rangle}}%
\@pvspace{8.0pt}%
 \@x{ hasSeenMostRecentStateFrom ( j ,\, n ) \.{\defeq} state . node [ n ]
 \.{\in} ToSet ( state . join [ j ] )}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
\@x{}%
...
\@xx{}%
\@x{ MessageTypes \.{\defeq} \{\@w{Current\_state} \}}%
\@pvspace{8.0pt}%
\@x{ Messages \.{\defeq}}%
 \@x{\@s{16.4} [ from\@s{0.25} \.{:} Pairs ( \{\@w{node} \} ,\, nodes
 )\@s{4.10} \.{\cup} Pairs ( \{\@w{join} \} ,\, joining )}%
 \@x{\@s{16.4} ,\, to\@s{12.78} \.{:} Pairs ( \{\@w{node} \} ,\, nodes )
 \.{\cup} Pairs ( \{\@w{join} \} ,\, joining )}%
\@x{\@s{16.4} ,\, msg \.{:} MessageTypes ]}%
\@pvspace{8.0pt}%
 \@x{ Msg ( from ,\, to ,\, msg ) \.{\defeq} [ from \.{\mapsto} from ,\, to
 \.{\mapsto} to ,\, msg \.{\mapsto} msg ]}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
\@x{}%
...
\@x{}%
\@y{\@s{0}%
 Requesting peers from the \ensuremath{DNS
}}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 Joining nodes in the \ensuremath{\@w{init}} phase request peers and
 transition to the \ensuremath{\@w{handshake}} phase
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ RequestPeers ( j ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} j \.{\in} init}%
 \@x{\@s{16.4} \.{\land} phase \.{'} \.{=} [ phase {\EXCEPT} {\bang} [ j ]
 \.{=}\@w{handshake} ]}%
 \@x{\@s{16.4} \.{\land} peers \.{'}\@s{2.35} \.{=} [ peers {\EXCEPT} {\bang}
 [ j ]\@s{2.35} \.{=} PickOfSize ( nodes ,\, peerThreshold ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} state ,\, joined ,\, secured
 ,\, sent ,\, recv {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Once peers have been obtained (\ensuremath{i.e}. the joining node is in the
 \ensuremath{\@w{handshake}} phase
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 and they are \ensuremath{peerSaturated}), these joining nodes attempt to
 handshake with their peers
}%
\@xx{}%
\@x{ Handshake ( j ,\, n ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} j\@s{1.87} \.{\in} handshaking}%
\@x{\@s{16.4} \.{\land} n \.{\in} peers [ j ]}%
\@x{\@s{16.4} \.{\land} {\lnot} connected ( j ,\, n )}%
\@x{\@s{16.4} \.{\land} {\lnot} connectionSaturated ( j )}%
 \@x{\@s{16.4} \.{\land} secured \.{'} \.{=} [ secured {\EXCEPT} {\bang} .
 join [ j ] \.{=} @ \.{\cup} \{ n \} ,\, {\bang} . node [ n ] \.{=} @
 \.{\cup} \{ j \} ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} state ,\, joined ,\, peers ,\,
 phase ,\, sent ,\, recv {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Once a handshaking node is \ensuremath{connectionSaturated}, they start
 bootstrapping
}%
\@xx{}%
\@x{ Transition ( j ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} j \.{\in} handshaking}%
\@x{\@s{16.4} \.{\land} connectionSaturated ( j )}%
 \@x{\@s{16.4} \.{\land} phase \.{'} \.{=} [ phase {\EXCEPT} {\bang} [ j ]
 \.{=}\@w{bootstrap} ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} state ,\, joined ,\, peers ,\,
...
\@x{}%
\@y{\@s{0}%
 Once a joining node is bootstrapping, they can request state from their
 connections
}%
\@xx{}%
\@x{ Bootstrap ( j ,\, n ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} j \.{\in} bootstrapping}%
\@x{\@s{16.4} \.{\land} connected ( j ,\, n )}%
\@x{\@s{16.4} \.{\land} {\lnot} hasSeenMostRecentStateFrom ( j ,\, n )}%
 \@x{\@s{16.4} \.{\land} sent \.{'} \.{=} [ sent {\EXCEPT} {\bang} . node [ n
 ] \.{=} @ \.{\cup} Msg ( {\langle}\@w{join} ,\, j {\rangle} ,\,
 {\langle}\@w{node} ,\, n {\rangle} ,\,\@w{Current\_state} ) ]}%
 \@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} joined ,\, peers ,\, phase ,\,
 secured ,\, state ,\, recv {\rangle}}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ Join ( j ) \.{\defeq}}%
\@x{\@s{19.81} \.{\land} saturatedAndBootstrapped ( j )}%
 \@x{\@s{19.81} \.{\land} phase \.{'} \.{=} [ phase {\EXCEPT} {\bang} [ j ]
 \.{=}\@w{joined} ]}%
\@x{\@s{19.81} \.{\land} joined \.{'} \.{=} joined \.{\cup} \{ j \}}%
 \@x{\@s{19.81} \.{\land} {\UNCHANGED} {\langle} state ,\, peers ,\, secured
 {\rangle}}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
...
\@x{}%
\@y{\@s{0}%
 All \ensuremath{init} nodes have no peers.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
 All non-\ensuremath{init} nodes are peer saturated.
}%
\@xx{}%
\@x{ PeersOK \.{\defeq}}%
\@x{\@s{16.4} \.{\land} \A\, j \.{\in} init \.{:} peers [ j ] \.{=} \{ \}}%
 \@x{\@s{16.4} \.{\land} \A\, j \.{\in} handshaking \.{\cup} bootstrapping
 \.{\cup} joined \.{:}}%
\@x{\@s{31.61} \.{\land} peers [ j ] \.{\neq} \{ \}}%
\@x{\@s{31.61} \.{\land} peerSaturated ( j )}%
 \@x{\@s{16.4} \.{\land} peers \.{\in} [ joining \.{\rightarrow} {\SUBSET}
 nodes ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Joining nodes can only be in one of the four specified phases:
 \ensuremath{\@w{init}},\ensuremath{\@w{handshake}},\ensuremath{\@w{bootstrap}},\ensuremath{\@w{joined}}.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ MutuallyDisjointPhases \.{\defeq}}%
\@x{\@s{16.4} \.{\land} disjoint ( init ,\, handshaking )}%
\@x{\@s{16.4} \.{\land} disjoint ( init ,\, bootstrapping )}%
\@x{\@s{16.4} \.{\land} disjoint ( init ,\, joined )}%
\@x{\@s{16.4} \.{\land} disjoint ( handshaking ,\, bootstrapping )}%
\@x{\@s{16.4} \.{\land} disjoint ( handshaking ,\, joined )}%
\@x{\@s{16.4} \.{\land} disjoint ( bootstrapping ,\, joined )}%
\@pvspace{8.0pt}%
\@x{ PhaseOK \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} joining \.{=} init \.{\cup} handshaking \.{\cup}
 bootstrapping \.{\cup} joined}%
\@x{\@s{16.4} \.{\land} MutuallyDisjointPhases}%
 \@x{\@s{16.4} \.{\land} \A\, j \.{\in} joining \.{:} phase [ j ]
 \.{=}\@w{joined} \.{\equiv} j \.{\in} joined}%
 \@x{\@s{16.4} \.{\land} phase \.{\in} [ joining \.{\rightarrow}
 PossiblePhases ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@x{}%
\@y{\@s{0}%
 \ensuremath{Init} and handshaking nodes must have an empty seq of states.
}%
\@xx{}%
\@x{ InitHandshakeEmptyStateOK \.{\defeq}}%
 \@x{\@s{16.4} \A\, j \.{\in} init \.{\cup} handshaking \.{:} state . join [ j
 ] \.{=} {\langle} {\rangle}}%
\@pvspace{8.0pt}%
\@x{ BootstrapStateOK \.{\defeq}}%
\@x{\@s{16.4} \A\, j \.{\in} bootstrapping \.{:}}%
 \@x{\@s{27.72} state . join [ j ] \.{\neq} {\langle} {\rangle} \.{\implies}
 \E\, n \.{\in} nodes \.{:} hasSeenMostRecentStateFrom ( j ,\, n )}%
\@pvspace{8.0pt}%
\@x{ JoinedStateOK \.{\defeq}}%
\@x{\@s{16.4} \A\, j \.{\in} joined \.{:}}%
\@x{\@s{27.72} \.{\land} state . join [ j ] \.{\neq} {\langle} {\rangle}}%
 \@x{\@s{27.72} \.{\land} \E\, n \.{\in} nodes \.{:}
 hasSeenMostRecentStateFrom ( j ,\, n )}%
\@pvspace{8.0pt}%
\@x{ StateOK \.{\defeq}}%
\@x{\@s{16.4} \.{\land} InitHandshakeEmptyStateOK}%
\@x{\@s{16.4} \.{\land} BootstrapStateOK}%
\@x{\@s{16.4} \.{\land} JoinedStateOK}%
 \@x{\@s{16.4} \.{\land} state \.{\in} [ join\@s{6.38} \.{:} [ joining
 \.{\rightarrow} Seq ( ValidStates ) ]}%
\@x{\@s{62.03} ,\, node \.{:} [ nodes \.{\rightarrow} ValidStates ] ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Established nodes must have sufficiently many connections to other
 established and non-\ensuremath{init} nodes.
}%
\@xx{}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ SecuredJoinOK \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} \A\, j \.{\in} init \.{:} secured . join [ j ] \.{=}
 \{ \}}%
 \@x{\@s{16.4} \.{\land} \A\, j \.{\in} handshaking \.{:} secured . join [ j ]
 \.{\subseteq} nodes}%
\@x{\@s{16.4} \.{\land} \A\, j \.{\in} bootstrapping \.{\cup} joined \.{:}}%
\@x{\@s{31.61} \.{\land} secured . join [ j ] \.{\neq} \{ \}}%
\@x{\@s{31.61} \.{\land} connectionSaturated ( j )}%
\@pvspace{8.0pt}%
\@x{ SecuredNodeOK \.{\defeq}}%
\@x{\@s{16.4} \A\, n \.{\in} nodes \.{:}}%
\@x{\@s{27.72} \.{\land} secured . node [ n ] \.{\neq} \{ \}}%
 \@x{\@s{27.72} \.{\land} connectionSaturatedFrom ( n ,\, ( nodes
 \.{\,\backslash\,} \{ n \} ) \.{\cup} ( joining \.{\,\backslash\,} init ) )}%
\@pvspace{8.0pt}%
\@x{ SecuredOK \.{\defeq}}%
\@x{\@s{16.4} \.{\land} SecuredJoinOK}%
\@x{\@s{16.4} \.{\land} SecuredNodeOK}%
 \@x{\@s{16.4} \.{\land} secured \.{\in} [ join\@s{6.38} \.{:} [ joining
 \.{\rightarrow} {\SUBSET} nodes ]}%
 \@x{\@s{72.92} ,\, node \.{:} [ nodes \.{\rightarrow} {\SUBSET} ( nodes
 \.{\cup} ( joining \.{\,\backslash\,} init ) ) ] ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Type invariant
...
\@xx{}%
\@x{ TypeOK \.{\defeq}}%
\@x{\@s{16.4} \.{\land}\@s{9.74} PeersOK}%
\@x{\@s{16.4} \.{\land}\@s{9.74} PhaseOK}%
\@x{\@s{16.4} \.{\land}\@s{9.74} StateOK}%
\@x{\@s{16.4} \.{\land}\@s{9.74} SecuredOK}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
\@x{}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- all joining nodes are in the \ensuremath{\@w{init}} phase
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 - all joining nodes have empty state and all established nodes have a valid
 state
\end{cpar}%
...
\end{lcom}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} phase \.{=} [ j \.{\in} joining \.{\mapsto}\@w{init}
 ]}%
 \@x{\@s{16.4} \.{\land} peers\@s{2.35} \.{=} [ j \.{\in} joining \.{\mapsto}
 \{ \} ]}%
\@x{\@s{16.4} \.{\land} joined \.{=} \{ \}}%
\@x{\@s{16.4} \.{\land} state \.{=}}%
 \@x{\@s{31.61} [ join\@s{6.38} \.{\mapsto} [ j \.{\in} joining \.{\mapsto}
 {\langle} {\rangle} ]}%
 \@x{\@s{31.61} ,\, node \.{\mapsto} \.{\LET} start \.{\defeq} Pick (
 ValidStates ) \.{\IN} [ n \.{\in} nodes \.{\mapsto} start ] ]}%
\@x{\@s{16.4} \.{\land} secured \.{=}}%
 \@x{\@s{31.61} [ join\@s{6.38} \.{\mapsto} [ j\@s{1.87} \.{\in} joining
 \.{\mapsto} \{ \} ]}%
 \@x{\@s{31.61} ,\, node \.{\mapsto} [ n \.{\in} nodes \.{\mapsto} PickOfSize
 ( nodes \.{\,\backslash\,} \{ n \} ,\, connectionThreshold ) ] ]}%
\@x{\@s{16.4} \.{\land} sent \.{=}}%
 \@x{\@s{31.61} [ join\@s{6.38} \.{\mapsto} [ j\@s{1.87} \.{\in} joining
 \.{\mapsto} \{ \} ]}%
\@x{\@s{31.61} ,\, node \.{\mapsto} [ n \.{\in} nodes \.{\mapsto} \{ \} ] ]}%
\@x{\@s{16.4} \.{\land} recv \.{=}}%
 \@x{\@s{31.61} [ join\@s{6.38} \.{\mapsto} [ j\@s{1.87} \.{\in} joining
 \.{\mapsto} {\langle} {\rangle} ]}%
 \@x{\@s{31.61} ,\, node \.{\mapsto} [ n \.{\in} nodes \.{\mapsto} {\langle}
 {\rangle} ] ]}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{InitRequestPeers}: request peers from \ensuremath{DNS
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{HandshakesHappen}: handshake with peers
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{TransitionHappen}: get ready to bootstrap
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{GettingBootstrap}: download state from \ensuremath{peer(s)
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{BootstrapperJoin}: join the network
\end{cpar}%
\end{lcom}%
\@pvspace{8.0pt}%
 \@x{ InitRequestPeers\@s{10.80} \.{\defeq} \E\, j \.{\in} init \.{:}
 RequestPeers ( j )}%
\@pvspace{8.0pt}%
 \@x{ HandshakesHappen \.{\defeq} \E\, j \.{\in} handshaking \.{:} \E\, n
 \.{\in} peers [ j ] \.{:} Handshake ( j ,\, n )}%
\@pvspace{8.0pt}%
 \@x{ TransitionHappen\@s{5.77} \.{\defeq} \E\, j \.{\in} handshaking \.{:}
 Transition ( j )}%
\@pvspace{8.0pt}%
 \@x{ GettingBootstrap\@s{9.81} \.{\defeq} \E\, j \.{\in} bootstrapping \.{:}
 \E\, n \.{\in} peers [ j ] \.{:} Bootstrap ( j ,\, n )}%
\@pvspace{8.0pt}%
 \@x{ BootstrapperJoin\@s{9.47} \.{\defeq} \E\, j \.{\in} bootstrapping \.{:}
 Join ( j )}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq}}%
\@x{\@s{16.4} \.{\lor} InitRequestPeers}%
\@x{\@s{16.4} \.{\lor} HandshakesHappen}%
\@x{\@s{16.4} \.{\lor} TransitionHappen}%
\@x{\@s{16.4} \.{\lor} GettingBootstrap}%
\@x{\@s{16.4} \.{\lor} BootstrapperJoin}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
 Weak fairness of \ensuremath{Next} actions
}%
\@xx{}%
\@x{ WFairness \.{\defeq}}%
\@x{\@s{16.4} \.{\land} {\WF}_{ vars} ( InitRequestPeers )}%
\@x{\@s{16.4} \.{\land} {\WF}_{ vars} ( HandshakesHappen )}%
\@x{\@s{16.4} \.{\land} {\WF}_{ vars} ( TransitionHappen )}%
\@x{\@s{16.4} \.{\land} {\WF}_{ vars} ( GettingBootstrap )}%
\@x{\@s{16.4} \.{\land} {\WF}_{ vars} ( BootstrapperJoin )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
Spec:
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- Initialized with \ensuremath{Init
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{WF} of \ensuremath{Next} actions
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
- \ensuremath{Next} are the only non-stuttering actions
\end{cpar}%
\end{lcom}%
 \@x{ Spec \.{\defeq} Init \.{\land} WFairness \.{\land} {\Box} [ Next ]_{
 vars}}%
\@pvspace{8.0pt}%
\@x{}\midbar\@xx{}%
\@pvspace{8.0pt}%
...
\@xx{}%
\@x{ AllNodesJoined \.{\defeq} joining \.{=} joined}%
\@pvspace{8.0pt}%
\@x{}%
\@y{\@s{0}%
...
\@xx{}%
\@x{ AllNodesHaveSameState \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} \A\, j \.{\in} joining \.{:} \A\, n \.{\in} nodes
 \.{:} state . join [ j ] \.{=} state . node [ n ]}%
 \@x{\@s{16.4} \.{\land} \A\, i ,\, j\@s{6.73} \.{\in} joining \.{:} state .
 join [ i ] \.{=} state . join [ j ]}%
 \@x{\@s{16.4} \.{\land} \A\, m ,\, n \.{\in} nodes \.{:} state . node [ m ]
 \.{=} state . node [ n ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{lcom}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} Safety \.{\defeq} Spec \.{\implies} {\Box} TypeOK}%
\@pvspace{8.0pt}%
 \@x{ {\THEOREM} Liveness \.{\defeq} Spec \.{\implies} {\Diamond} {\Box} (
 AllNodesJoined \.{\land} AllNodesHaveSameState )}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\end{document}
