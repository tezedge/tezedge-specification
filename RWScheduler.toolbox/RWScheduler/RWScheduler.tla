...
RECURSIVE sum(_)
sum(f) ==
    IF Cardinality(DOMAIN f) = 0
    THEN 0
    ELSE LET x == CHOOSE _x \in DOMAIN f : TRUE
             g == [ y \in (DOMAIN f \ {x}) |-> f[y] ]
    IN f[x] + sum(g)
...
Handle ==
    ...
    /\ handle' = [ handle EXCEPT
                     ...
                     !.msg = Tail(@) ]          \* removed the handled message
...
(* When we are not handling data or already waiting for data, we can wait for data *)
Wait_data ==
    /\ ~handle.b
    /\ ~waiting
    /\ queuesEmpty
...
(* When waiting for data, we get a msg from in_param and place it in the appropriate queue *)
Waiter(id) ==
    /\ waiting
    /\ in_param[id] # <<>>
    /\ IF queuesEmpty
...
(* When the scheduler quota < 0, we can update the quota and rearrange our queues *)
Update_quota ==
    /\ quota < 0
    ...
    /\ UNCHANGED <<state, counter, waiting, connections, handle, in_param>>
